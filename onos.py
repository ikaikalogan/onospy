...
from mininet.node import Controller, OVSSwitch, UserSwitch, OVSController, RemoteController, CPULimitedHost, Host, Node, \
...
class hybridBattalion(Topo):
    ...
    def __init__(self):
        ...
        s5 = self.addSwitch('s5', cls=LinuxBridge)
...
class Battalion(Topo):
    ...
    def __init__(self):
        ...
        self.addLink(h7, s6)
        self.addLink(h1, s3)
        # lower link is for adding tests
        # self.addLink(s3, s6)
...
# topos = { 'none': Topo }
topos = {'mytopo': (lambda: MyTopo()), 'hybridbattalion': (lambda: hybridBattalion()),
         'battalion': (lambda: Battalion()), 'stcut': (lambda: Stcut())}
